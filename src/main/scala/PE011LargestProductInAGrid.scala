 ...
 */
object PE011LargestProductInAGrid {

  val problemGrid = List( List( 8,  2, 22, 97, 38, 15,  0, 40,  0, 75,  4,  5,  7, 78, 52, 12, 50, 77, 91,  8),
                    ...
                    List(04, 52,  8, 83, 97, 35, 99, 16,  7, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66),
  ...
  def maxInRow(length: Int, row: List[Int]): Long = {
    ...
    row.sliding(length).map(
      s => {
        val r = if (s.length == length) s.reduceLeft(_ * _) else 0
        //println("   Evaluating segment: " + s + ", max = " + r)
        r
      }
    ).max
  }
  ...
  def maxX(length: Int, grid: List[List[Int]]): Long = {
    //println("Evaluating grid: " + grid)
    val v = grid.map(maxInRow(length, _))
    v match {
      case List() => 0
      case _ => v.max
    }
  ...
  def maxY(length: Int, grid: List[List[Int]]): Long = {
  ...
  def maxDiagonals(length: Int, grid: List[List[Int]]): Long = {
    grid.map(row => {
      // we only need the bit of the grid that extends from row to the
      // length of the sequence we're looking for
      val rowIndex = grid.indexOf(row)
      val subGrid = grid.slice(rowIndex, rowIndex + length)
      Math.max(maxSEDiagonals(length, subGrid), maxSWDiagonals(length, subGrid))
    }).max
  }

  def largestProduct(): Long = {
    val length = 4
    List(maxX(length, problemGrid), maxY(length, problemGrid), maxDiagonals(length, problemGrid)).max
  }
